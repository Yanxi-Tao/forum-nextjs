...
export default function CreatePostPage({ params }: { params: { info: string[] } }) {
  ...
  return (
    <Card className="border-0 shadow-none">
      <CardHeader>
        ...
        <CardDescription>{`Community: ${communityName ? communityName : 'General'}`}</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs className="w-full" defaultValue={postType}>
          <TabsList className="w-full">
            <TabsTrigger value="question" className="w-full" asChild>
              <Link href={`/create/question/${communityName ?? ''}`}>Question</Link>
            </TabsTrigger>
            <TabsTrigger value="article" className="w-full" asChild>
              <Link href={`/create/article/${communityName ?? ''}`}>Article</Link>
            </TabsTrigger>
